...
import { servicesFromGatewayArr, isGatewayArrEnabled } from "utils/config/gateway-arr";
...
export async function servicesResponse() {
  ...
  let discoveredGatewayArrServices;
  ...
  // Gateway-arr integration - fetch services from operator API
  try {
    if (isGatewayArrEnabled()) {
      discoveredGatewayArrServices = cleanServiceGroups(await servicesFromGatewayArr());
      if (discoveredGatewayArrServices?.length > 0) {
        console.debug(`Discovered ${discoveredGatewayArrServices.length} service groups from Gateway-arr`);
      }
    } else {
      discoveredGatewayArrServices = [];
    }
  } catch (e) {
    console.error("Failed to discover services from Gateway-arr operator.");
    if (e) console.error(e.toString());
    discoveredGatewayArrServices = [];
  }
  ...
  const mergedGroupsNames = [
    ...new Set(
      [
        ...
        discoveredGatewayArrServices.map((group) => group.name),
  ...
  mergedGroupsNames.forEach((groupName) => {
    ...
    const discoveredGatewayArrGroup = findGroupByName(discoveredGatewayArrServices, groupName) || {
      services: [],
    };
    ...
    const mergedGroup = {
      ...
      services: [
        ...
        ...discoveredGatewayArrGroup.services,
